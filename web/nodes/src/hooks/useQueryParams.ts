...
const useQueryParameters = (initialParameters: QueryParameters, call: string, optionalPayload?: {[key: string]: any}) => {
  ...
  const sort = (sortProps: SortProps) => {
    const updatedQueryParams = {...queryParameters.value, orderBy: sortProps.sortField, order: sortProps.sortOrder === 1 ? 'asc' as 'asc' : 'desc' as 'desc'}
    queryParameters.value = updatedQueryParams
    store.dispatch(call, optionalPayload ? { ...payload.value, queryParameters: updatedQueryParams } :  updatedQueryParams)
  }
